...
async def process_images_in_folder(folder_path, output_csv, text_prompt, max_concurrent_requests=5):
    ...
    with open(output_csv, mode="w", newline="") as csv_file:
        ...
        csv_writer.writerow(["Image ID", "Urban", "Rural", "Uninhabited"])
...
if __name__ == "__main__":
    folder_path = "./48RYV_n_times_n"
    output_csv = "pseudo_labels_" + folder_path.split("/")[-1] + ".csv"
    text_prompt = "你是一个卫星图像专家和经济学专家。我将给你一张卫星图片，你需要关于它属于城市地区、农村地区还是无人居住区打分，分数越高表明越属于这一类。你需要输出一个列表，列表的第一、二、三个元素为属于城市、农村、无人区的分数，每一项都是0到100的值。输出时不要附带任何解释信息，只输出形如[a,b,c]的分数列表。"
